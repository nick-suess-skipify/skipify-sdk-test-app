...
export class Base {
    ...
    positionIframe(shouldScroll = false) {
        ...
        if (this.shouldDisplayOnTop) {
            this.messenger.iframe.style.bottom = window.innerHeight - buttonPosition.top + 16 + 'px';
        } else {
            this.messenger.iframe.style.bottom = '';
        }

        const { width: iframeWidth } = this.messenger.iframe.getBoundingClientRect();
    ...
    insertButton(emailInput: HTMLElement, overrideStyles: Record<string, string | number> = {}) {
        ...
        for (const property of stylePropertiesToCopy) {
            ...
            if (borderRadius) {
                ...
                this.button?.style.setProperty(property, `${buttonSize / borderRadiusValue}%`);
            }
        }
    ...
    getEmbedContainer(): HTMLElement | undefined {
        console.warn('-- getEmbedContainer should be overwritten by platform class');
        return undefined;
    }
