...
describe('TTLStorage', () => {
    ...
    it('should retrieve an unexpired item', () => {
        ...
        const value = { data: 'testValue' };
        const now = Date.now();
        mockStorage.getItem.mockReturnValue(JSON.stringify({ value, cat: now, ttl: 1000 }));
    ...
    it('should update the TTL of an existing item', () => {
        ...
        expect(mockStorage.setItem).toHaveBeenCalledWith(key, JSON.stringify({ value, cat: now + 500, ttl: 2000 }));
    });
