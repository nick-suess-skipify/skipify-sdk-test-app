...
export class BigCommerceSDK extends Base implements AbstractSDK {
    ...
    processLoggedInCustomer() {
        const loggedInCustomerElem = document.querySelector(this.loggedInCustomerSelector);
        if (!loggedInCustomerElem || loggedInCustomerElem?.classList.contains(SkipifyClassNames.loggedInCustomer)) {
            return;
        }
    ...
    async fetchUserEmailFromCart(onlyGuest = true) {
        ...
        if (onlyGuest && userCart.customerId !== 0) {
        ...
        try {
            if (userCart.id && userCart.customerId !== 0) {
                // Only do that for logged in users: customerId != 0
    ...
    override async getUserEnrollmentInformation() {
        ...
        // https://support.bigcommerce.com/s/question/0D54O00006sUx6PSAS/can-you-get-the-order-id-using-javascript-on-the-order-confirmation-page
        const completedOrderElement = document.querySelector(this.completedOrderSelector);
        const completedOrderId = completedOrderElement ? completedOrderElement.textContent : null;
