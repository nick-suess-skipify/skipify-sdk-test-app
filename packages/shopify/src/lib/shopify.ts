...
class ShopifySDK extends Base implements AbstractSDK {
    ...
    processEmailInput(): void {
        ...
        if (!!emailInputElem.value && emailInputElem.value === userEmail) {
            ...
            log('Email input value is same as userEmail, user most likely refreshed the page');
        } else {
    ...
    processEnrollmentCheckbox(): void {
        const paymentButtonElem = document.querySelector(this.paymentButtonSelector) as HTMLElement;
